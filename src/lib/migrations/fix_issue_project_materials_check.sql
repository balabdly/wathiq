-- إصلاح صرف العهدة: INSERT بـ qty_received=0 يفشل CHECK قبل ON CONFLICT
-- الحل: UPDATE مباشر لعمليات الصرف/الإرجاع، INSERT فقط للاستلام
...
begin
  ...
  for v_line in select * from jsonb_array_elements(p_payload -> 'lines') loop
    ...
    if v_project_id is not null and v_voucher_type <> 'TRANSFER' then
      ...
      elsif v_voucher_type in ('RETURN_WH','RETURN_CLIENT') then
