...
create table if not exists project_planning (
  ...
  materials_list_file_name      text,
  material_receipt_type         text default 'full' check (material_receipt_type in ('full', 'partial')),
  material_receipt_notes        text,
  material_delay_client_caused  boolean default false,
  material_delay_revised_end    date,
  permit_number               text,
  permit_start                date,
  permit_end                  date,
