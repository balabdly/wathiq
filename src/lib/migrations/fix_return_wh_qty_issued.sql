-- مرتجع الموقع (RETURN_WH): يُخفّض qty_issued (مواد عادت من الموقع) وليس qty_returned
...
begin
  ...
  for v_line in select * from jsonb_array_elements(p_payload -> 'lines') loop
    ...
    v_pm_wh_id := coalesce(v_warehouse_id, v_mat.warehouse_id);
    ...
    if v_project_id is not null and v_voucher_type <> 'TRANSFER' then
      ...
      if v_voucher_type = 'RECEIVE' then
        ...
        values (v_tenant_id, v_project_id, v_mat_id, v_pm_wh_id, v_qty, 0, 0, v_voucher_no)
        on conflict (tenant_id, project_id, material_id, warehouse_id) do update set
          qty_received = project_materials.qty_received + v_qty,
          last_recv_txn = v_voucher_no,
          updated_at = now();
      elsif v_voucher_type = 'ISSUE' then
        update project_materials set qty_issued = qty_issued + v_qty, updated_at = now()
        where tenant_id = v_tenant_id and project_id = v_project_id and material_id = v_mat_id and warehouse_id = v_pm_wh_id
          and (qty_received - qty_issued - qty_returned) >= v_qty;
        if not found then raise exception 'رصيد مشروع غير كافٍ للمادة "%"', v_mat.name; end if;
      elsif v_voucher_type = 'RETURN_WH' then
        update project_materials set qty_issued = greatest(0, qty_issued - v_qty), updated_at = now()
        where tenant_id = v_tenant_id and project_id = v_project_id and material_id = v_mat_id and warehouse_id = v_pm_wh_id
          and qty_issued >= v_qty;
        if not found then raise exception 'كمية الإرجاع تتجاوز المصروف للمادة "%"', v_mat.name; end if;
      elsif v_voucher_type = 'RETURN_CLIENT' then
        update project_materials set qty_returned = qty_returned + v_qty, updated_at = now()
        where tenant_id = v_tenant_id and project_id = v_project_id and material_id = v_mat_id and warehouse_id = v_pm_wh_id
          and (qty_received - qty_issued - qty_returned) >= v_qty;
        if not found then raise exception 'رصيد مشروع غير كافٍ للمادة "%"', v_mat.name; end if;
      end if;
