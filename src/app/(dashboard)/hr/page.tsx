...
import {
  Users, Plus, Search, Pencil, X, Save,
...
function LeaveModal({ leave, hrEmployees, onClose, onSave }: {
  leave: Leave | null; hrEmployees: HREmployee[]; onClose: () => void; onSave: (d: any) => Promise<void>
}) {
  const [saving, setSaving] = useState(false)
  const [form, setForm] = useState({
    employee_id: leave?.employee_id || '', leave_type: leave?.leave_type || 'سنوية',
    start_date: leave?.start_date || '', end_date: leave?.end_date || '',
    reason: leave?.reason || '', status: leave?.status || 'بانتظار الموافقة',
  })
  const set = (k: string, v: any) => setForm(f => ({ ...f, [k]: v }))
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            <div><label className="block text-sm font-medium text-gray-700 mb-1.5">الموظف <span className="text-red-500">*</span></label>
              <select value={form.employee_id} onChange={e => set('employee_id', e.target.value)} className="select" required>
                <option value="">— اختر موظف —</option>
                {hrEmployees.map(e => <option key={e.employee_id} value={e.employee_id}>{e.employee?.name}</option>)}
              </select>
            </div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1.5">نوع الإجازة</label>
            ...
            {days > 0 && <div style={{ background: 'var(--primary-light)', borderRadius: '10px', padding: '10px 14px', textAlign: 'center' }}><span style={{ fontWeight: 700, color: 'var(--primary)', fontSize: '1.1rem' }}>{days} يوم</span></div>}
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            <div className="grid grid-cols-3 gap-3">
              <div style={{ gridColumn: '1/-1' }}><label className="block text-sm font-medium text-gray-700 mb-1.5">الموظف</label>
                <select value={form.employee_id} onChange={e => handleEmpChange(e.target.value)} className="select" required>
                  <option value="">— اختر موظف —</option>
                  {hrEmployees.map(e => <option key={e.employee_id} value={e.employee_id}>{e.employee?.name}</option>)}
                </select>
              </div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1.5">الشهر</label>
          ...
          <div className="modal-footer">
            <button type="button" onClick={onClose} className="btn btn-ghost">إلغاء</button>
            <button type="submit" disabled={saving} className="btn btn-primary">
              {saving ? <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Save className="w-4 h-4" />} حفظ كشف الراتب
            </button>
          </div>
        </form>
      </div>
...
export default function HRPage() {
  ...
  async function loadData() {
    ...
    setLoading(true)
    const tid = tenant.id
    if (activeTab === 'employees') { const { data } = await supabase.from('hr_employees').select('*, employee:employees(name, role, username)').eq('tenant_id', tid).order('id'); setHREmployees(data || []) }
    else if (activeTab === 'leaves') { const { data } = await supabase.from('hr_leaves').select('*, employee:employees(name)').eq('tenant_id', tid).order('start_date', { ascending: false }); setLeaves(data || []) }
    else if (activeTab === 'attendance') { const { data } = await supabase.from('hr_attendance').select('*, employee:employees(name)').eq('tenant_id', tid).order('date', { ascending: false }).limit(100); setAttendance(data || []) }
    else if (activeTab === 'payroll') { const { data } = await supabase.from('hr_payroll').select('*, employee:employees(name, role)').eq('tenant_id', tid).order('year', { ascending: false }).order('month', { ascending: false }); setPayrolls(data || []) }
    else if (activeTab === 'documents') { const { data } = await supabase.from('hr_documents').select('*, employee:employees(name)').eq('tenant_id', tid).order('expiry_date'); setDocuments(data || []) }
    setLoading(false)
  }
  ...
  async function saveHREmployee(data: any) {
    ...
    await loadData(); setEmpModal(false); setEditEmp(null)
  ...
  async function saveLeave(data: any) {
    ...
    await loadData(); setLeaveModal(false); setEditLeave(null); toast.success('تم الحفظ ✅')
  }
  ...
  async function savePayroll(data: any) {
    ...
    await loadData(); setPayrollModal(false); setEditPayroll(null); toast.success('تم حفظ كشف الراتب ✅')
  }
  ...
  const expiringDocs = documents.filter(d => { if (!d.expiry_date) return false; const days = Math.ceil((new Date(d.expiry_date).getTime() - now.getTime()) / 86400000); return days <= 30 && days > 0 }).length
  ...
  const STATUS_COLOR: Record<string, string> = {
    ...
    'مسودة': 'badge-gray', 'معتمد': 'badge-blue', 'مدفوع': 'badge-green',
    'حضور': 'badge-green', 'غياب': 'badge-red', 'إجازة': 'badge-amber', 'مأمورية': 'badge-blue',
  }
    ...
    <div className="space-y-5 fade-in">
      <div>
        <h1 className="text-xl font-bold text-gray-800" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Users style={{ width: '20px', height: '20px', color: 'var(--primary)' }} /> الموارد البشرية
        </h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          <div key={kpi.label} className="card" style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: kpi.bg, color: kpi.color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{kpi.icon}</div>
            <div>
        ...
        {TABS.map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id as any)}
            style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 18px', borderRadius: '10px', fontSize: '0.875rem', fontWeight: 600, border: 'none', cursor: 'pointer', transition: 'all 0.2s',
              background: activeTab === t.id ? 'var(--primary)' : 'transparent', color: activeTab === t.id ? 'white' : 'var(--text3)',
      ...
      {activeTab === 'employees' && (
        <div className="space-y-4">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
            ...
            {isAdmin && <button onClick={() => { setEditEmp(null); setEmpModal(true) }} className="btn btn-primary"><Plus style={{ width: '16px', height: '16px' }} /> إضافة موظف</button>}
          </div>
          ...
          : hrEmployees.length === 0 ? (
            <div className="card" style={{ padding: '60px', textAlign: 'center' }}>
              ...
              {isAdmin && <button onClick={() => { setEditEmp(null); setEmpModal(true) }} className="btn btn-primary" style={{ marginTop: '16px' }}><Plus style={{ width: '16px', height: '16px' }} /> إضافة أول موظف</button>}
            </div>
          ) : (
                  ...
                  <div key={emp.id} className="card" style={{ padding: '20px' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: 'var(--primary-light)', color: 'var(--primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '1.1rem' }}>{emp.employee?.name?.charAt(0)}</div>
                        <div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '12px', fontSize: '0.8rem' }}>
                      <div><span style={{ color: 'var(--text3)' }}>الجنسية: </span><span style={{ fontWeight: 600 }}>{emp.nationality}</span></div>
                      <div><span style={{ color: 'var(--text3)' }}>العقد: </span><span style={{ fontWeight: 600 }}>{emp.contract_type}</span></div>
                      {emp.hire_date && <div style={{ gridColumn: '1/-1' }}><span style={{ color: 'var(--text3)' }}>تاريخ التعيين: </span><span style={{ fontWeight: 600 }}>{formatDate(emp.hire_date)}</span></div>}
                    </div>
                    ...
                    {iqamaDays !== null && iqamaDays <= 60 && (
                      <div style={{ background: iqamaDays <= 0 ? '#fef2f2' : '#fffbeb', borderRadius: '8px', padding: '8px', marginBottom: '10px', fontSize: '0.75rem', color: iqamaDays <= 0 ? '#c81e1e' : '#e6820a', display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <AlertTriangle style={{ width: '14px', height: '14px' }} />
                        {iqamaDays <= 0 ? 'إقامة منتهية!' : `إقامة تنتهي خلال ${iqamaDays} يوم`}
                      </div>
                    )}
                    {isAdmin && (
      ...
      {activeTab === 'attendance' && (
        <div className="space-y-4">
          ...
          : <div className="table-wrap"><table><thead><tr><th>الموظف</th><th>التاريخ</th><th>الحالة</th><th>ساعات العمل</th><th>الإضافي</th></tr></thead>
            <tbody>{attendance.map(a => <tr key={a.id}><td style={{ fontWeight: 600 }}>{a.employee?.name || `#${a.employee_id}`}</td><td>{formatDate(a.date)}</td><td><span className={`badge ${STATUS_COLOR[a.status] || 'badge-gray'}`}>{a.status}</span></td><td style={{ textAlign: 'center' }}>{a.hours_worked || '—'}</td><td style={{ textAlign: 'center' }}>{a.overtime_hours || '—'}</td></tr>)}</tbody>
            </table></div>}
        </div>
      )}
      ...
      {activeTab === 'leaves' && (
        <div className="space-y-4">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
            ...
            <button onClick={() => { setEditLeave(null); setLeaveModal(true) }} className="btn btn-primary"><Plus style={{ width: '16px', height: '16px' }} /> طلب إجازة</button>
          </div>
          {loading ? <div style={{ display: 'flex', justifyContent: 'center', padding: '40px' }}><div className="w-8 h-8 border-2 border-primary-500/30 border-t-primary-500 rounded-full animate-spin" /></div>
          : leaves.length === 0 ? <div className="card" style={{ padding: '60px', textAlign: 'center' }}><Calendar style={{ width: '48px', height: '48px', color: 'var(--border)', margin: '0 auto 12px' }} /><p style={{ color: 'var(--text3)' }}>لا توجد طلبات إجازة</p></div>
          : <div className="table-wrap"><table><thead><tr><th>الموظف</th><th>النوع</th><th>من</th><th>إلى</th><th>الأيام</th><th>الحالة</th><th></th></tr></thead>
            <tbody>{leaves.map(l => <tr key={l.id}>
              ...
              <td>{isAdmin && <button onClick={() => { setEditLeave(l); setLeaveModal(true) }} className="btn btn-ghost btn-xs"><Pencil style={{ width: '14px', height: '14px' }} /></button>}</td>
            </tr>)}</tbody></table></div>}
        </div>
      )}
      ...
      {activeTab === 'payroll' && (
        <div className="space-y-4">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
            ...
            {isAdmin && <button onClick={() => { setEditPayroll(null); setPayrollModal(true) }} className="btn btn-primary"><Plus style={{ width: '16px', height: '16px' }} /> إنشاء كشف راتب</button>}
          </div>
          {loading ? <div style={{ display: 'flex', justifyContent: 'center', padding: '40px' }}><div className="w-8 h-8 border-2 border-primary-500/30 border-t-primary-500 rounded-full animate-spin" /></div>
          : payrolls.length === 0 ? <div className="card" style={{ padding: '60px', textAlign: 'center' }}><Banknote style={{ width: '48px', height: '48px', color: 'var(--border)', margin: '0 auto 12px' }} /><p style={{ color: 'var(--text3)' }}>لا توجد كشوف رواتب بعد</p></div>
          : <div className="table-wrap"><table><thead><tr><th>الموظف</th><th>الشهر</th><th>الإجمالي</th><th>الخصومات</th><th>الصافي</th><th>الحالة</th><th></th></tr></thead>
            <tbody>{payrolls.map(p => <tr key={p.id}>
              ...
              <td style={{ color: '#0ea77b', fontWeight: 700 }}>{p.net_salary.toLocaleString()} ر.س</td>
              <td><span className={`badge ${STATUS_COLOR[p.status] || 'badge-gray'}`}>{p.status}</span></td>
              <td>{isAdmin && <button onClick={() => { setEditPayroll(p); setPayrollModal(true) }} className="btn btn-ghost btn-xs"><Pencil style={{ width: '14px', height: '14px' }} /></button>}</td>
            </tr>)}</tbody></table></div>}
        </div>
      )}
      ...
      {activeTab === 'documents' && (
        <div className="space-y-4">
          ...
          {loading ? <div style={{ display: 'flex', justifyContent: 'center', padding: '40px' }}><div className="w-8 h-8 border-2 border-primary-500/30 border-t-primary-500 rounded-full animate-spin" /></div>
          : documents.length === 0 ? <div className="card" style={{ padding: '60px', textAlign: 'center' }}><FileText style={{ width: '48px', height: '48px', color: 'var(--border)', margin: '0 auto 12px' }} /><p style={{ color: 'var(--text3)' }}>لا توجد وثائق مضافة</p></div>
          : <div className="table-wrap"><table><thead><tr><th>الموظف</th><th>نوع الوثيقة</th><th>الاسم</th><th>الرقم</th><th>تاريخ الانتهاء</th><th>الحالة</th></tr></thead>
            <tbody>{documents.map(d => {
              ...
              return <tr key={d.id} style={{ background: isExpired ? '#fff5f5' : isSoon ? '#fffbeb' : '' }}>
                <td style={{ fontWeight: 600 }}>{d.employee?.name}</td>
      ...
      {showEmpModal && <HREmployeeModal emp={editEmp} employees={employees} onClose={() => { setEmpModal(false); setEditEmp(null) }} onSave={saveHREmployee} />}
      {showLeaveModal && <LeaveModal leave={editLeave} hrEmployees={hrEmployees} onClose={() => { setLeaveModal(false); setEditLeave(null) }} onSave={saveLeave} />}
      {showPayrollModal && <PayrollModal payroll={editPayroll} hrEmployees={hrEmployees} onClose={() => { setPayrollModal(false); setEditPayroll(null) }} onSave={savePayroll} />}
    </div>
  )
}
