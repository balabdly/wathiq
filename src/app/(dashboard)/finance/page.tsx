    ...
    try {
      ...
      if (group === 'invoices') {
        const [inv, cn] = await Promise.all([
          supabase.from('finance_invoices').select('*, finance_clients(name)').eq('tenant_id', tid).order('invoice_date', { ascending: false }),
      ...
      if (group === 'purchases') {
        const [po, vi] = await Promise.all([
          supabase.from('finance_purchase_orders').select('*, finance_vendors(name)').eq('tenant_id', tid).order('created_at', { ascending: false }),
          supabase.from('finance_vendor_invoices').select('*, finance_vendors(name)').eq('tenant_id', tid).order('invoice_date', { ascending: false }),
        ])
