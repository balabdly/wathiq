// src/app/(dashboard)/finance/expenses/receipts/page.tsx
'use client'
...
export default function ReceiptsPage() {
  ...
  async function loadVouchers() {
    ...
    const { data } = await supabase.from('finance_treasury')
      ...
      .eq('tenant_id', tenantId).eq('type', 'قبض')
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <button onClick={() => setShowVoucher(true)} className="btn btn-primary" style={{ background: '#0ea77b' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
        {[
          { label: 'إجمالي المقبوضات', value: totalVouchers, color: '#0ea77b', bg: '#0ea77b11' },
      ...
      {loading ? (
        ...
        <div className="card" style={{ padding: '60px', textAlign: 'center' }}>
          ...
          <p style={{ color: '#9ca3af' }}>لا توجد سندات قبض بعد</p>
        </div>
      ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
              <thead>
                <tr style={{ background: 'var(--bg2)', borderBottom: '2px solid var(--border)' }}>
                  {['رقم السند', 'التاريخ', 'البيان', 'المُدفِع', 'المشروع', 'الحساب', 'المبلغ', 'الحالة', ''].map(h => (
              ...
              <tbody>
                {vouchers.map(v => (
                  <tr key={v.id} style={{ borderBottom: '1px solid var(--bg2)' }}
                    ...
                    <td style={{ padding: '10px 12px', fontWeight: 700, color: '#0ea77b', whiteSpace: 'nowrap' }}>
                      +{fmt(v.amount)} ر.س
                    </td>
              ...
              <tfoot>
                <tr style={{ borderTop: '2px solid var(--border)', background: 'var(--bg2)', fontWeight: 700, fontSize: '0.82rem' }}>
                  ...
                  <td style={{ padding: '10px 12px', color: '#0ea77b' }}>{fmt(totalVouchers)} ر.س</td>
                  <td colSpan={2} />
                </tr>
              </tfoot>
            </table>
      ...
      {showVoucher && (
        <VoucherModal type="قبض" cashAccounts={cashAccounts} accounts={accounts} costCenters={costCenters} clients={clients} vendors={vendors} projects={projects} tenantId={tenantId!}
